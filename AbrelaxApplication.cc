...
void AbrelaxApplication::fold( core::pose::Pose &init_pose, ProtocolOP prot_ptr ) {
	...
	while ( jobdist.next_job(curr_job, curr_nstruct) && !bEndrun ) {
        ...
        // AQUI ESTA LLAMANDO A CLASSIC ABINITIO
        // leer el ultimo pdb y meterlo en una variable que seria vector_soluciones_anteriores
        // copiar a abnitio_protocol con algo tipo:
        // abinitio_protocol.soluciones_anteriores = vector_soluciones_anteriores
        // ya puedes ejecutar tal cual
        //
        ...
        if (umbralLimite > 0) {
            ...
            if(paths_soluciones_pdbs.size()>0){
                if(soluciones_anteriores.size()>0){
                    std::vector<std::string>::iterator it = paths_soluciones_pdbs.end();
